...
All query topologies must contain an `.originPartition` call (= back to the query's original task) as the final partitioner of the computation. The `.out` must be emitted exactly once.
