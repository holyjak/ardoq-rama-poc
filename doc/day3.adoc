...
* After I implemented the `ancestors` query returning a components ancestors, I wrote `ancestor?` for checking whether a component is an ancestor of another from scratch, using a recursive query, as this was much simpler.
