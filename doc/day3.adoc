...
* How to select on a potentially different partition and then come back?
...
(bar :a> <aaa> *v1 *v2 ; emit ??? to the stream a, anchor aaa
:b> <anchor-b> *v2 ; emit once, anchor anchor-b
:> *a) ; emit once to the default stream
